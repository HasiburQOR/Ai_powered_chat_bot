...
- **`platforms/`** — `Channel` model (Instagram/Messenger/WhatsApp/WordPress connections + credentials). Named `platforms`, not `channels`, to avoid confusion with the unrelated Django Channels (websockets) library.
...
- **`webhooks/`** — Instagram/Messenger/WhatsApp webhook receiver views (thin — verify, parse, enqueue, return 200 fast). All three Meta surfaces share the one `/webhooks/meta/` endpoint.
...
### WhatsApp
Same endpoint and handshake as above, but the payload shape differs (`object == "whatsapp_business_account"`, `entry[].changes[].value.messages[]`) and the channel is identified by `metadata.phone_number_id` instead of a page id — details in `04_API_WEBHOOKS_SPEC.md`. Outbound goes to `/<phone_number_id>/messages` with a Bearer access token; the bot's `**bold**` is converted to WhatsApp's `*bold*` and long replies are split into ≤4096-char bubbles. Free-form replies are only allowed within 24 hours of the customer's last message (the bot always replies immediately after an inbound, so this holds by construction). Inbound is text-only in v1 — image/audio/etc. messages are acked and ignored.
...
- **Short-term memory:** last N messages (configurable, default 50 — deep recall, ~4k tokens/call on long chats) for the current `Conversation`, pulled straight from the `Message` table and formatted as the chat history sent to the LLM.
- **Long-term memory:** `Customer.memory_summary` — a plain-text summary regenerated by a Celery task every time a conversation crosses a message-count threshold (every 10 customer messages — deliberately below the short-term window, so context leaving the window is already captured in the summary and nothing falls into a gap) or goes idle. The task asks the LLM to summarize "durable facts worth remembering about this customer" from the recent transcript and overwrites the field. This keeps the prompt small instead of ever-growing.
