...
| `memory_summary` | TextField, blank | Regenerated by Celery: on the message-count trigger AND by the beat-driven idle summarizer |
| `memory_summary_at` | DateTime, null | When `memory_summary` was last regenerated; lets the idle summarizer skip already-current customers |
| `created_at` / `updated_at` | DateTime | |
